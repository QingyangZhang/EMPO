...
async def parallel_compute_score_async(evaluation_func, completions, references, tasks, extra_info=None, num_processes=64):
    ...
    for result, completion, reference, task in zip(results, completions, references, tasks):
        if isinstance(result, Exception) or result is None:
            ...
            extracted_gts.append(result[1])
            extracted_preds.append(result[2])

    return scores, extracted_gts, extracted_preds
...
class PrimeRewardManager:
    ...
    def verify(self, data):
        ...
        try:
            scores, extracted_gts, extracted_preds = asyncio.run(
            ...
            extracted_gts = [None for _ in range(len(sequences_str))]
            extracted_preds = [None for _ in range(len(sequences_str))]
        except Exception as e:
        ...
        data.non_tensor_batch['extracted_gts'] = extracted_gts
        data.non_tensor_batch['extracted_preds'] = extracted_preds
        return scores
