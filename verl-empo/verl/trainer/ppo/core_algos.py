...
@register_adv_est(AdvantageEstimator.GRPO_ENT) # or simply: @register_adv_est("grpo_ent")
def compute_grpo_ent_outcome_advantage(
    token_level_rewards: torch.Tensor,
    token_level_entropys: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    p: float = 0.5,
    epsilon: float = 1e-6,
    ...
    For each group of samples with the same index:
    1. Normalizes scores to get advantages.
    2. For positive advantage samples, it keeps the advantages of the top p-proportion
       of samples with the highest entropy, keeping at least one.
    3. For negative advantage samples, it keeps the advantages of the top p-proportion
       of samples with the lowest entropy, keeping at least one.
    ...
    Args:
        ...
        response_mask: `(torch.Tensor)`
            shape is (bs, response_length)
        index: `(np.ndarray)`
            Array of indices to group samples, shape is (bs,)
        p: `(float)`
            The proportion (0-1) of samples to keep for both positive and
            negative advantage groups. Defaults to 0.5.
        epsilon: `(float)`
    ...
    # Ensure no division by zero for empty sequences
    entropys = (token_level_entropys * response_mask).sum(dim=-1) / seq_lengths

    # This will hold the final, filtered advantages
    final_advantages = torch.zeros_like(scores)

    with torch.no_grad():
        # Group samples by their index
        id_to_samples = defaultdict(list)
        bsz = scores.shape[0]
        for i in range(bsz):
            id_to_samples[index[i]].append({
                "original_idx": i,
                "score": scores[i],
                "entropy": entropys[i],
            })
        ...
        # Process each group
        for idx in id_to_samples:
            samples = id_to_samples[idx]
            group_scores = torch.tensor([s["score"] for s in samples])

            # Calculate mean and std for the group
            if len(samples) > 1:
                mean_score = torch.mean(group_scores)
                std_score = torch.std(group_scores)
            else:
                # If only one sample, advantage is 0, but we process it for consistency
                mean_score = torch.tensor(0.0)
                std_score = torch.tensor(1.0)
                
            # Calculate advantages for all samples in the group
            for s in samples:
                if norm_adv_by_std_in_grpo:
                    s["advantage"] = (s["score"] - mean_score) / (std_score + epsilon)
                else:
                    s["advantage"] = s["score"] - mean_score

            # Separate into positive and negative samples based on advantage
            pos_samples = [s for s in samples if s["advantage"] > 0]
            neg_samples = [s for s in samples if s["advantage"] <= 0]

            # For positive samples, keep the top p-proportion with the highest entropy
            if pos_samples:
                # Sort descending by entropy
                pos_samples_sorted = sorted(pos_samples, key=lambda s: s["entropy"], reverse=True)
                # Calculate how many to keep based on proportion p, with a minimum of 1
                num_to_keep_pos = max(1, math.ceil(p * len(pos_samples)))
                # Keep the advantages for the selected samples
                for sample_to_keep in pos_samples_sorted[:num_to_keep_pos]:
                    final_advantages[sample_to_keep["original_idx"]] = sample_to_keep["advantage"]

            # For negative samples, keep the top p-proportion with the lowest entropy
            if neg_samples:
                # Sort ascending by entropy
                neg_samples_sorted = sorted(neg_samples, key=lambda s: s["entropy"], reverse=True)
                # Calculate how many to keep based on proportion p, with a minimum of 1
                num_to_keep_neg = max(1, math.ceil(p * len(neg_samples)))
                # Keep the advantages for the selected samples
                for sample_to_keep in neg_samples_sorted[:num_to_keep_neg]:
                    final_advantages[sample_to_keep["original_idx"]] = sample_to_keep["advantage"]

    # Expand the advantages to the token level using the response mask
    advantages_at_token_level = final_advantages.unsqueeze(-1) * response_mask

    return advantages_at_token_level, advantages_at_token_level.clone()
...
def compute_policy_loss_gspo_token(
    ...
    normalized_old_seq_log_prob = torch.sum(old_log_prob * response_mask, dim=-1) / seq_lengths
    negative_approx_kl_seq = normalized_seq_log_prob - normalized_old_seq_log_prob
    seq_ratio = torch.exp(negative_approx_kl_seq).detach()
    ...
    seq_ratio_expanded = seq_ratio.unsqueeze(1)  # (batch_size, 1)
...
def self_certainty_penalty(logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor, kl_penalty) -> torch.FloatTensor:
    """Compute KL divergence given logprob and ref_logprob.
    Copied from https://github.com/huggingface/trl/blob/main/trl/trainer/ppo_trainer.py#L1104
    See more description in http://joschu.net/blog/kl-approx.html
