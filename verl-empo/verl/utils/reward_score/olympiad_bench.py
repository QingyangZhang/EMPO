...
class AutoScoringJudge:
    ...
    def judge(self, expression1, expression2, precision=1e-8):
        ...
        if expression1 == expression2:
            # print("Exactly equal")
            return True, expression1, expression2
...
def compute_score(model_output: str, ground_truth: str, precision=1e-4) -> bool:
    scorer = AutoScoringJudge()
    acc, extracted_gt, extracted_pred = scorer.judge(ground_truth, model_output, precision)
    return {
        "score": float(acc),
        "acc": acc,
