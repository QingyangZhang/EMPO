...
def compute_score(model_output: str, ground_truth: str, fast=False):
    extracted_pred = model_output
    if "<answer>" in extracted_pred and "</answer>" in extracted_pred:
        extracted_pred = extracted_pred.split("<answer>")[-1].replace("</answer>", "")
    ...
    if "\\boxed" in extracted_pred:
        extracted_pred = extract_answer(extracted_pred)
    ...
    if isinstance(ground_truth, str):
        is_correct = grade(extracted_pred, ground_truth, fast)
    elif isinstance(ground_truth, list):
        is_correct = False
        for gt in ground_truth:
            is_correct |= grade(extracted_pred, gt, fast)
    if is_correct:
        return {
            "score": 1.0,
            "acc": True,
