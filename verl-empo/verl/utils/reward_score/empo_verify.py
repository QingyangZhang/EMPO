...
class Model_args:
    ...
    base_url = 'http://10.102.247.20:34886/v1'  # Anonymized model path or URL
    max_tokens = 4096
    temperature = 0.1
...
'''
    ...
    # print(f"[Debug] Output results: {correct}")
    
    return correct.strip() == "Correct" or correct.strip() == "[Correct]"
...
def grade(model_answer: str, gt_answer: str, is_matched: bool, problem=None, use_xverify=False):
...
def get_answer_str(s: str, return_origin=False, num_answers=1):
    ...
    if not return_origin:
        answer_list = [a if a else "" for a in answer_list]
...
def answer_tag_reward_fn_for_r1(model_output: str, ground_truths, problem=None, points=None, use_xverify=False):
    ...
    for extracted_pred, ground_truth in zip(extracted_answers, ground_truths):
        score, score_by, extracted_pred, extracted_gt = grade(extracted_pred, ground_truth, is_matched, problem, use_xverify=use_xverify)
...
def compute_score(model_output: str, ground_truths: str, question: str, points, use_xverify):
    ...
    score, point, extracted_pred, extracted_gt, scored_by = answer_tag_reward_fn_for_r1(model_output, ground_truths, question, points, use_xverify=True)
...
def grade_single_answer(extracted_answer: str, 
          gt_answer: str, 
          question=None, 
          model_args: Model_args = Model_args()):

    # print(f'trying to grade {extracted_answer}, {gt_answer}')
    
    assert isinstance(extracted_answer, str), f"answer must be str but got {type(extracted_answer).__name__}"
    assert isinstance(gt_answer, str), f"ground truth must be str but got {type(gt_answer).__name__}"

    # gt_answer = solution2answer(gt_answer, return_origin=True)[0]
    
    if extracted_answer == '':
        return 0.0

    if gt_answer == '':
        # print('[Warning] Ground truth is an empty String. Return 0 reward.')
        return 0.0

    try:
        correct, score_by, extracted_pred, extracted_gt = grade(extracted_answer, gt_answer, is_matched=True, problem=question, use_xverify=False)
    except Exception:
        e = traceback.format_exc()
        print(f"[Error] Grade {extracted_answer}, {gt_answer} failed due to {e}, return 0 reward")
        correct = 0.0
...
if __name__ == "__main__":
    # Example usage
    model_output = r"The answer is <answer>$\boxed{2}$</answer>"
    ground_truth = ["2.0"]
    question = "What is the answer to 2+1?"
    ...
    result = grade_single_answer(model_output, ground_truth)
    print(result)  # Should print a dictionary with score and correctness information.