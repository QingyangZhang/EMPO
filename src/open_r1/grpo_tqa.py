...
class GRPOScriptArguments(ScriptArguments):
    ...
    dataset_usage: float = field(
        default=None,
        metadata={"help": "Maximum samples for training"},
    )
...
def main(script_args, training_args, model_args):
    ...
    #if script_args.max_train_samples and len(dataset) > script_args.max_train_samples:
    #    #print("We only use {} samples for training.".format(max_train_samples))
    #    selected_indices = random.sample(range(len(dataset)), script_args.max_train_samples)
    #    dataset = dataset.select(selected_indices)
    if 'truthful' in script_args.dataset_name:
        max_train_samples = int(len(dataset) * script_args.dataset_usage)
    else:
        max_train_samples = script_args.max_train_samples
        dataset = dataset.select(range(max_train_samples))
    print('Totoal train samples: {}'.format(len(dataset)))
    ...
    def make_conversation(example):
        ...
        if 'trivia_qa' in script_args.dataset_name:
            prompt.append({"role": "user", "content": "Answer the following question in one single sentence as briefly as possible. Q: {}\n A:".format(example["question"])})
        else:
            prompt.append({"role": "user", "content": "Answer the following question in one single sentence as briefly as possible. Q: {}\n A:".format(example["Question"])})
    ...
    if "messages" in dataset.column_names:
        dataset[split] = dataset.remove_columns("messages")
